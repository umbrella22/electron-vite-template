...
if (process.env.NODE_ENV !== 'development') {
  ...
  process.env.__lib = join(__dirname, '..', '..', '..', '..', `rootLib`).replace(/\\/g, '\\\\')
  process.env.__common = join(__dirname, '..', '..', '..', '..', 'common').replace(/\\/g, '\\\\')
  process.env.__updateFolder = join(__dirname, '..', '..', '..', '..', `${HotUpdateFolder}`).replace(/\\/g, '\\\\')
} else {
  process.env.__static = join(__dirname, '..', '..', '..', 'static').replace(/\\/g, '\\\\')
  process.env.__updateFolder = join(__dirname, '..', '..', '..', `${HotUpdateFolder}`).replace(/\\/g, '\\\\')
  process.env.__lib = join(__dirname, '..', '..', '..', `rootLib`,`${process.platform}`,`${process.arch}`).replace(/\\/g, '\\\\')
  process.env.__common = join(__dirname, '..', '..', '..', `rootLib`, `common`).replace(/\\/g, '\\\\')
}
...
export const common = process.env.__common
